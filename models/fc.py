...
class FC:
    ...
    def set_model_vars(self, sess, variables):
        ...
        for attr, var in variables.items():
            vars.append(var)
        feed_dict = {
            self.weight_ph:vars[0],
            self.bias_ph:vars[1]
        }
        sess.run([self.update_weight,self.update_bias],feed_dict = feed_dict)

    def obtain_model_vars(self, sess):
        weights, biases = sess.run([self.weights, self.biases])
        weight = weights.copy()
        bias = biases.copy()
        return {'weights': weight, 'biases': bias}

    def assign_model_vars(self, sess, variables):
        for attr, var in variables.items():
            tensor = getattr(self, attr)
    ...
    def construct_vars(self):
        ...
        self.bias_ph = tf.placeholder(
            tf.float32,
            shape=(self.ensemble_size, 1, self.output_dim),
            name='weight',
        )

        self.update_weight = self.weights.assign(self.weight_ph)
        self.update_bias = self.biases.assign(self.bias_ph)
