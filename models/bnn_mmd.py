...
class BNN:
    ...
    def finalize(self, optimizer, optimizer_args=None, kernel_sigma=None, *args, **kwargs):
        ...
        with tf.variable_scope('fine_tune'):
            self.optimizer_adapt = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5)
            ...
            for i in range(self.num_nets):
                kerGI = mmd._mix_rbf_kernel(h_s[i], h_t[i], sigmas=kernel_sigma)
                mmd_loss += mmd.mmd2(kerGI)
            mmd_loss = tf.sqrt(tf.maximum(mmd_loss, 1e-6))
            self.train_op_adapt = self.optimizer_adapt.minimize(mmd_loss + tf.add_n(self.decays_adapt), var_list=self.optvars_adapt + self.optvars)

        # Initialize all variables
        self.sess.run(tf.variables_initializer(
            self.optvars + self.optvars_adapt + self.nonoptvars + self.optimizer.variables() + self.optimizer_adapt.variables()))
    ...
    def adapt(self, source_input, target_input, batch_size=32, max_steps=200):
