...
from models.constructor_mmd import construct_model, format_samples_for_training
...
class AMPO_MMD(RLAlgorithm):
    ...
    def __init__(
        ...
        super(AMPO_MMD, self).__init__(**kwargs)
        ...
        self._model = construct_model(obs_dim=obs_dim, act_dim=act_dim, hidden_dim=hidden_dim,
                                      num_networks=num_networks, num_elites=num_elites, kernel_sigma=kernel_sigma)
    ...
    def _train(self):
        ...
        for self._epoch in range(self._epoch, self._n_epochs):
            ...
            for i in count():
                ...
                if self._timestep % self._model_train_freq == 0:
                    ...
                    if self._epoch <= self._epoch_stop_adapt:
                        print('[Adaptation] Begin finetune the model')
                        self._set_n_adapt()
                        self._adapt_model(batch_size=self._adapt_batch_size, max_steps=self._n_adapt)
            ...
            # f_log.write('model validation loss: %f\n' % model_metrics['val_loss'])
    ...
    def _adapt_model(self, batch_size=256, max_steps=200):
        ...
        self._model.adapt(source_inputs, target_inputs, batch_size, max_steps)
